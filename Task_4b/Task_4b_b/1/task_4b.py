...
## modules for this task (np, opencv)                    ##
##############################################################
import socket
import time
import os, sys
...
#################################  ADD YOUR CODE HERE  ###############################
    ...
    eulerAngles_robot = [1.5708,yaw,+1.5708]
...
def emulation(event,sim):
    ...
    while(True):
        ...
        print(res[0],res[5],res[6])

        Storage_log.data.append(res[0])

        if res[0] != Storage_log.data[-1]:

	        if res[5] == "DELIVERED" :
	        	place_packages(sim,res[6],res[0],res[5])
...
def task_4b_implementation(sim,config_img,medicine_package_details):
	"""
	Purpose:
	---
	...
	#shared_message = ['', '', '', '', 0, ''] ### empty message ; length = 6

	event = Event()
	med_pack  = "None"
	try:
		Thread1 = Thread(target = emulation,args=(event,sim))
		Thread1.start()
		print("Thread 1 started")
	except :
		Thread1 = Thread(target = emulation,args=(event,sim,))
		Thread1.start()
		print("Re trying to start thread 1")
		print(error)
				...
				while True:
					...
					if message:
						...
						if type(message) == list:
							...
							xy = pb_theme.position_data(message[0])
							scene_parameters = [xy[0],xy[1],0]
							#set_values(sim,scene_parameters)
							
							activity_log_message = str(message[1])+ ": " + str(message[0])
							pb_theme.send_message_via_socket(connection_1,activity_log_message)
							print(activity_log_message)
...
if __name__ == "__main__":
	...
	task_4b_implementation(sim,config_img,medicine_package_details)

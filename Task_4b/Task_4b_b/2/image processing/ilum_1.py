...
image_main = cv2.imread("road_ref.jpeg")
image = image_main.copy()

darkchannel,brightchannel = get_illumination_channel(image,w)
atmosphere_lightning = get_atmosphere(image,brightchannel)
initial_transmission = get_initial_transmission(atmosphere_lightning,brightchannel)
corrected_transmission = get_corrected_transmission(image,atmosphere_lightning,darkchannel,brightchannel,initial_transmission,alpha,omega,w)
result = get_final_image(image,atmosphere_lightning,corrected_transmission,0.5)

cv2.imshow("image_main",image_main)
#cv2.imshow("darkchannel",darkchannel)
#cv2.imshow("brightchannel",brightchannel)
#cv2.imshow("atmosphere_lightning",atmosphere_lightning)
#cv2.imshow("initial_transmission",initial_transmission)
#cv2.imshow("corrected_transmission",corrected_transmission)

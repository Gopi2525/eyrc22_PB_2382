...
if __name__ == "__main__":
		...
		while 1 :
			...
			if st == 2:
				state = 0
				ps = 0
				package_status_i = "No package"
				package_status_f = "No package"
				if colour != "Turn off":
					rgb_led_set_color(colour)
					package_status_i = "PICKED UP"
					package_status_f = "DELIVERED"
				while 1:
					### status message
					if state == sizef(moves):
						if sizef(paths) != sizef(moves):
							k = "ARRIVED AT"
					elif moves[state] == 'WAIT_5':
						k = "WAIT AT"
					else:
						k = "ARRIVED AT"
					time.sleep(2)
					if state == sizef(paths)-1:
						status = [paths[state],k,"-","-",1,package_status_f]
						send_message_via_socket(client, json.dumps(status))
						rgb_led_set_color("Turn off")
						print(status)
						break
					else:
						if ps != 0 and colour != "Turn off" :
							package_status_i = "Carrying"
						ps = 1
						status = [paths[state],k,paths[state],paths[state+1],0,package_status_i]
						send_message_via_socket(client, json.dumps(status))
						print(status)
						state = state + 1
			if st == 2 and s == 1:

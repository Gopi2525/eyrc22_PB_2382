...
def change_new_lane(sim,cmds_nodes,img2):
	...
	if cmds_nodes == 20:
		...
		sim.callScriptFunction("activate_qr_code", childscript_handle, Storage.cmds_checkpoints[Storage.cmds_checkpoints_state])
		...
		while 1 :
			...
			package_name = read_qr_code(sim)

			if package_name != None :

				if package_name == "Orange Cone" :

					p_n = "package_1"

				elif package_name == "Blue Cylinder" :

					p_n = "package_2"

				elif package_name == "Pink Cuboid" :
				...
				#sim.callScriptFunction("deliver_package", childscript_handle, "package_1", Storage.cmds_checkpoints[Storage.cmds_checkpoints_state])

				## Deliver package_1 at checkpoint E
				sim.callScriptFunction("deliver_package", childscript_handle, p_n, Storage.cmds_checkpoints[Storage.cmds_checkpoints_state])

				print(f"Dropped {package_name} at {Storage.cmds_checkpoints[Storage.cmds_checkpoints_state]} ")

				break

		## Call the deactivate_qr_code() function defined in the child script to make the QR code invisible at checkpoint E
		sim.callScriptFunction("deactivate_qr_code", childscript_handle, Storage.cmds_checkpoints[Storage.cmds_checkpoints_state])

		Storage.cmds_checkpoints_state = Storage.cmds_checkpoints_state +1
...
def yellowchecker(sim,jointHandle_l,jointHandle_r):
	...
	if Storage.yellow[-1] != 0 and Storage.yellow[-2] == 0 and Storage.cmds_nodes[Storage.cmds_nodes_state] == 100 :
	...
	elif Storage.yellow[-1] == 0 and Storage.yellow[-2] != 0 and Storage.cmds_nodes[Storage.cmds_nodes_state] != 100 :
	...
	if Storage.yellow[-2] != 0 and Storage.yellow[-1] == 0  :
